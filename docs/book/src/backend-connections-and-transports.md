...
| Upstream backend | Shared `reqwest::Client` plus RMCP `StreamableHttpClientTransport`. | `common.rs` and `gateway/mcp_gateway.rs`. | Open MCP client sessions to configured backend MCP servers. |
