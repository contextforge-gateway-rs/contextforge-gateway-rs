...
`call_tool`, `read_resource`, and `get_prompt` all share the same splitter. The
splitter checks configured backend names and requires the separator `-` after
the backend prefix. This avoids accepting names such as `gateway-oneincrement`.
