...
#[tokio::test(flavor = "multi_thread", worker_threads = 1)]
#[test_log::test]
async fn plaintext_call_prefixed_backend_tools_modern_modern_with_filtering() -> Result<()> {
    let gateway_port = create_ports(1)[0];
    ...
    let mut call_params = CallToolRequestParams::default();
    call_params.name = expected_tool_names[0].clone().into();
    let maybe_passed = assert_tools_call(
        gateway_url.clone(),
        client.clone(),
        call_params,
        "-1".to_owned(),
    ...
    let mut call_params = CallToolRequestParams::default();
    call_params.name = "random_tool_name".into();
    let maybe_not_passed =
        assert_tools_call(gateway_url, client, call_params, "-1".to_owned(), ProtocolVersion::V_2026_07_28).await;
...
async fn plaintext_call_prefixed_backend_tools_legacy_modern() -> Result<()> {
    ...
    call_params.name = expected_tool_names[0].clone().into();
    let maybe_passed =
        assert_tools_call(gateway_url, client, call_params, "-1".to_owned(), ProtocolVersion::V_2025_11_25).await;
