...
pub(crate) fn require_plugin_binding<'a>(
    revision: &str,
    plugins: Option<&'a [String]>,
) -> Result<&'a [String], ErrorData> {
    if revision.is_empty() {
        tracing::warn!("rejecting call with empty runtime plugin binding revision");
        return Err(ErrorData::internal_error("Runtime plugin binding is invalid", None));
    }
    plugins.ok_or_else(|| {
...
mod tests {
    ...
    fn plugin_bindings_resolve_only_exact_canonical_backend_targets() {
        let bindings = PluginBindings {
            ...
            tools: HashMap::from([(
                "backend-a".to_owned(),
                HashMap::from([("canonical-tool".to_owned(), vec!["validator-a".to_owned()])]),
            )]),
            resources: HashMap::new(),
            prompts: HashMap::new(),
        };

        assert_eq!(Some(["validator-a".to_owned()].as_slice()), bindings.tool_plugins("backend-a", "canonical-tool"));
    ...
    fn missing_or_unversioned_plugin_bindings_fail_closed() {
        assert!(require_plugin_binding("", Some(&[])).is_err());
        assert!(require_plugin_binding("revision-1", None).is_err());
        assert!(require_plugin_binding("revision-1", Some(&[])).is_ok());
    }
