...
#[ignore = "spawns redis-server and the contextforge-gateway-rs binary"]
...
fn start_gateway_process(gateway_port: u16, redis_port: u16) -> ChildProcess {
    let binary = env!("CARGO_BIN_EXE_contextforge-gateway-rs");
    let child = Command::new(binary)
        ...
        .stderr(Stdio::null())
        .spawn()
        .expect("contextforge-gateway-rs binary starts");
    ChildProcess::new("contextforge-gateway-rs", child)
}
